...
class WindowsInput extends Input {
			...
			child.stdout.on('data', async (data) => {
				...
				let jslines = await Promise.all(
					lines.map(l=>new Promise((ok,rej)=>{
						xml(l,(err,json)=>err?rej(err):ok(json));
					}))
				);

				jslines.forEach(json=>{
	...
	async next(callback) {
		let read = false;
		while(!read) {
			let item = await new Promise((ok,rej)=>this.queue.pop((err,item)=>ok(item),1000));
			if(!item) {
				this.fetch();
			}
			else {

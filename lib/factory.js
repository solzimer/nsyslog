...
function transform(instance,flow) {
	...
	let rfilter = def.disabled? null : getFilter(def.retry);
	let rfilterMax = def.retry.max || 0;
	let rfilterTimeout = def.retry.timeout || 0;
	let listeners = {in:new Set(),ack:new Set(),out:new Set()};
	...
	let prfn = async function(ok,rej) {
		let entry = entries.shift();
		let retry = rfilterMax;

		// Prefiltrado de la entrada
		if(filter && filter(entry)) {
			if(filterMatch==FILTER_ACTION.bypass) {
				ok(entry);
				notify(PROC_MODE.ack,entry);
		...
		// Proceso de entrada con reintentos
		do {
			// Procesar la entrada
			let r = await new Promise(async(ok)=>{
				try {
					await instance.process(entry,(err,res)=>ok({err,res}));		
				}catch(err) {
					ok({err});
				}				
			});

			// Evaluar si es necesario el reintento
			if(rfilter && rfilter(r.res)) {
				logger.debug(`${instance.id}: Next retry: ${retry} / ${rfilterMax}`);
				retry--;
			}
			else {
				retry = 0;
			...
			if(retry == 0) {
				...
				await new Promise(ok=>setTimeout(ok,rfilterTimeout));
			}
		}while(retry > 0);
	};

	// El stream duplex real que será devuelto
	let tr = new Duplex({
		objectMode : true,
		highWaterMark : instance.maxPending || instance.$def.maxPending,
		async write(entry,encoding,callback) {
	...
	tr._buffpop = function(err,entry) {
		...
		// Normalize entries
		let entries = (Array.isArray(entry)? entry : [entry]).filter(Boolean);
		if(!entry) {
			logger.error(`Null entry on processor ${instance.id}!`);
			throw new Error(`Null entry on processor ${instance.id}!`);
		}
		else {
			entries.forEach(item=>notify(PROC_MODE.output,item));
			entries.forEach(item=>tr.push(item));
		}
	};

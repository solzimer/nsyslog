...
async function read(file,callback,options) {
	...
	try {
		...
		if(typeof(file)=='object') {
			json = file;
			jsonErrors = await registerComponents(path.resolve(''),json.$path,json,jsonErrors,json.$filepath);
		...
		Config.events.emit('rawjson',json);
		let raw = JSON.parse(JSON.stringify(json));

		// Replace global properties ("@{....}" expressions)
		for(let i=0;i<5;i++) {
		...
		// Decrypt properties
		expression.traverse(json,(obj,key,val)=>{
			if(typeof(val)=='string' && val.indexOf("#{")>=0) {
				let match = val.match(/#\{([^\}]*)\}/)[1];
				try {
					return TLS.decrypt({},match);
				}catch(err) {
					logger.error(err);
					logger.warn(`Ignoring bad encrypted data "${match}"`);
					return val;
				}
			}
			else if(typeof(val)=='string' && val.indexOf("${AES:")>=0) {
				try {
					let enc = val.replace(/\$\{AES:/,'').replace(/\}/,'');
					let dec = aes.decrypt(enc);
					return dec;
				}catch(err) {
					logger.error(err);
					logger.warn(`Ignoring bad encrypted AES data "${val}"`);
					return val;
				}		
			}
			else {
		...
		// Replace extenders ({"$extend":...} expressions)
		let xts = JSONPath({path: '$..[?(@.$extend)]', json});
		xts.forEach(xt=>{
			let elems = [true].concat(xt.$extend);
			let res = extend.apply(this,elems);
			extend(xt,res);
			delete xt.$extend;
		});

		let realDataDir = json.config && json.config.datadir ? json.config.datadir : path.join(os.tmpdir(), 'nsyslog-db'+process.pid);
		if(process.env['INSTANCE']) {
			realDataDir = path.join(realDataDir,`instance_${process.env['INSTANCE']}`);
		}
		json.config = extend(true,{},DEF_CONF,json.config);
		json.config.datadir = getDataAccess(realDataDir);
...
async function imports(file,json,errors) {
	...
	res.forEach(njson=>{
		...
		let registers = njson.register;
		delete njson.flows;
		delete njson.register;
		extend(true,json,njson);
		json.flows = [].concat(json.flows||[]).concat(flows||[]);
		json.register = [].concat(json.register||[]).concat(registers||[]);
	});
...
async function registerComponents(basepath,ipath,json,errors,ifile) {
	...
	let all = comps.map(async(cmp)=>{
		...
		try {
			...
			await register(ipath,cmp);
		}catch(err) {
			//Config.event.emit('error',err);
...
function sanitizeModules(json) {
	...
	let oretry = {filter:false,max:0, timeout:0};
	...
	modules.forEach(modules=>{
		for(let i in modules) {
			...
			def.retry = extend({},oretry,def.retry);
		}
	});
}
...
function processFilters(json, errors) {
	...
	for(var i in json.filters) {
		...
		try {
			...
			Config.events.emit('error',new Error(`Error evaluation expression ${JSON.stringify(val)}`));

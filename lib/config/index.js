...
function processFilters(json, errors) {
	...
	json.flows.forEach(flow=>{
		if(!json.filters[flow.from] && /^[a-zA-Z0-9\$\#]+$/.test(flow.from))
...
class Config {
	...
	split() {
		...
		let flowmap = {};
		let master = pmap[MASTER_PROC].cfg;
		let assign = (id,flow)=>{
		...
		modules.flows.forEach(flow=>{
			...
			else if(flow.fork && flow.fparent) assign(flow.fparent,flow);
		...
		Object.keys(this.inputs).forEach(ki=>{
			...
			if(input.attach && input.attach.length) {
				...
				forked.forEach(kp=>{
					delete pmap[kp].cfg.inputs[ki];
					delete pmap[kp].cfg.$$raw.inputs[ki];
				});
			}

			// Filter master inputs
			Object.keys(modules.inputs).forEach(k=>{
				if(!master.inputs[k]) delete master.modules.inputs[k];
			});
		});
		...
		Object.keys(pmap).forEach(pk=>{
			...
			// Filter master processors
			Object.keys(modules.processors).forEach(k=>{
				if(!master.processors[k]) delete master.modules.processors[k];
			});
		});
		...
		Object.keys(pmap).forEach(pk=>{
			...
			let pset = new Set();
			pmap[pk].flows.forEach(flow=>{
				flow.transporters.list.forEach(p=>pset.add(p.id));
			});
			...
			// Filter master transporters
			Object.keys(modules.transporters).forEach(k=>{
				if(k.startsWith("#")) return;
				else if(!master.transporters[k])
					delete master.modules.transporters[k];
			});
		});
		...
		// Filter master flows
		master.modules.flows = pmap[MASTER_PROC].flows;

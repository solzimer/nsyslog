...
// Master process CLI
	...
	function init(vorpal, instance)  {
		...
		vorpal.
		  ...
		  action(function(){
				this.log(JSON.stringify(instance.config.$$raw,null,2));
			});
		...
		cluster.on(MODULE,(child,module,msg)=>{
			vorpal.log(`${msg.mode}:`.green, msg.entry);
		});
		...
		function fnsubscribe(type,args,callback) {
			...
			if(!mod) {
				...
				vorpal.log('\n********* Press Q to cancel *********\n\n');
				let fn = (mode,entry)=>vorpal.log(`${mode}:`.green, entry);
		...
		function fnstats(args, callback) {
			...
			if(isNaN(tival)) {
				vorpal.log(stats.all());
			}
			else {
				vorpal.log('\n********* Press Q to cancel *********\n\n');
				vorpal.log(stats.all());

				let ival = setInterval(()=>vorpal.log('stats'.yellow,stats.all()),tival*1000);

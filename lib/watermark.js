...
const MODE = {master:1,child:2,stateless:false};
...
class ChildDB extends WatermarkDB {
	...
	static getInstance(datadir, start) {
		if(!INSTANCES[datadir]) {
			let instance = new ChildDB(datadir);
...
if(cluster.isMaster) {
	cluster.on(MODULE,async(child,module,msg)=>{
		...
		let mdb = MasterDB.getInstance(datadir);
		await mdb.start();

		try {
			let res = await mdb[msg.cmd].apply(mdb,msg.args);
			let resp = {module:MODULE,mode:MODE.master,pid:msg.pid,cmd:CMD.success,cid:msg.cid,res};
			child.send(resp);
		}catch(err) {
			logger.error(err);

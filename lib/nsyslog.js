...
class NSyslog extends EventEmiter {
	...
	startProcessorStream() {
		...
		cfg.flows.forEach(flow=>{
			...
			if(!flow.fork) {
				...
				forEach(p=>{
					p.on(Events.data,this.handleEvent('data','processor',flow,p));
					p.on(Events.ack,this.handleEvent('ack','processor',flow,p));
					p.on(Events.error,this.handleEvent('error','processor',flow,p));
					p.on(Events.transfer,this.handleEvent('transfer','processor',flow,p));
					to = to.pipe(p);
				});
			}
			else {
				let p = FlowFork.fork(this.config.$path,flow.id);
				p.on(Events.data,this.handleEvent('data','fork',flow,p));
				p.on(Events.ack,this.handleEvent('ack','fork',flow,p));
				p.on(Events.error,this.handleEvent('error','fork',flow,p));
				p.on(Events.transfer,this.handleEvent('transfer','fork',flow,p));
				to = to.pipe(p);
			}
	...
	startTransportStream() {
		...
		function walk(trs,flow) {
			if(trs.transport) {
				...
				wr.on(Events.data,self.handleEvent('data','transporter',flow,wr));
				wr.on(Events.ack,self.handleEvent('ack','transporter',flow,wr));
				wr.on(Events.error,self.handleEvent('error','transporter',flow,wr));
				wr.on(Events.transfer,self.handleEvent('transfer','transporter',flow,wr));
				return wr;
			}
	...
	startInputs() {
		...
		Object.keys(cfg.modules.inputs).forEach(k=>{
			...
			input.on(Events.data,self.handleEvent('data','input',null,input));
			input.on(Events.ack,self.handleEvent('ack','input',null,input));
			input.on(Events.error,self.handleEvent('error','input',null,input));
			input.on(Events.transfer,self.handleEvent('transfer','input',null,input));

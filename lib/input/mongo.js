...
class MongoInput extends Input {
	...
	async fetchData(colname) {
		...
		try {
			...
			cursor = await this.db.collection(colname).find(query);
	...
	async connect() {
		...
		while(!connected) {
			try {
				...
				this.db.on('close', () => logger.warn(`${this.id}: MongoDB -> lost connection`,this.url));
				this.db.on('reconnect', () => logger.warn(`${this.id}: MongoDB -> reconnect`,this.url));

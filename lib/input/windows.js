...
class WindowsInput extends Input {
	...
	async read() {
        ...
        if(idfilter && idfilter.length>MAX_ID_FILTER && !idfilter._batches) {
            ...
            idfilter._batches = batches;
        }

        // Si tenemos batches, leemos por batches y vamos actualizando el offset después de cada batch
        let results = {records:[], lastRecordId: offset};
        let batchsize = this.batchsize;
        if(idfilter && idfilter._batches) {
            batchsize = Math.ceil(this.batchsize / idfilter._batches.length);
            for(let batch of idfilter._batches) {                
                logger.debug(`Reading events from channel ${channel} starting at offset ${offset} / batchsize ${batchsize} with idfilter ${batch}`);
                const partialResult = nwinread.readEvents(channel, READMODE.offset, offset, batchsize, batch);
                const partialLastOffset = partialResult.lastRecordId;
                results.records.push(...partialResult.records);
                results.lastRecordId = Math.max(results.lastRecordId, partialLastOffset);
                logger.silly("PARTIAL OFFSET",results.lastRecordId);
            }
        }
        else {
            logger.debug(`Reading events from channel ${channel} starting at offset ${offset} / batchsize ${batchsize} with idfilter ${idfilter}`);
            results = nwinread.readEvents(channel, READMODE.offset, offset, batchsize, idfilter);
        }

        const lastOffset = results.lastRecordId;
        logger.silly("LAST OFFSET",this.id,results.lastRecordId);
        for(let event of results.records) {
	...
	async next(callback) {
        ...
        try {
            ...
            if(this.channel!=parsed.Event.System.Channel) {
                logger.warn(`Channel mismatch: expected ${this.channel}, got ${parsed.Event.System.Channel} [EventRecordId: ${event.recordId}]`);
                logger.warn(`Event XML: ${event.xml}`);
            }

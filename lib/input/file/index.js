...
class FileInput extends Input {
	...
	async configure(config, callback) {
		...
		if(config.exclude)
			this.exclude = Path.resolve(config.$path, config.exclude);
	...
	async openFile(path) {
		...
		// Excluded files
		if(this.exlude && minimatch(path,this.exclude)) {
			...
			map(async(file)=>{
				try {
					...
					if(res.bytesRead==0) {
						...
						if(fstat.size < file.offset) {
							logger.warn(`File ${file.path} has been truncated. Reseting watermark`);
							file.offset = 0;
						}
						else {
							logger.debug(`Nothing to read from ${file.path}`);
							file.ready = false;
						}
					}

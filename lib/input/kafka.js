...
class KafkaInput extends Input {
	...
	async connect() {
		...
		logger.info('Connecting to kafka',hosts);
		...
		this.consumer.on('error',err=>{
			logger.error(err);
			logger.error('Error on kafka connection. Reconnecting...');
		});

		logger.info('Starting kafka metadata refresh');
		this.startIval();
	}
	...
	startIval() {
		...
		let fn = ()=>{
			i++;
			logger.debug('Refreshing kafka topics metadata');
			...
			if((i%5==0) && this.watch) {
				this.consumer.client.refreshBrokerMetadata((err)=>{
					...
					if(err) {
						return logger.error('Error refreshing topics',err);
					}
	...
	async start(callback) {
		...
		var reconnect = async()=>{
			try {
				await this.connect();
				logger.info('Kafka connection successfuly',this.url);
			}catch(err) {
				logger.error(err);
				logger.error('Error on kafka connection. Reconnecting...');

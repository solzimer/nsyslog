...
async function start(path, idFlow) {
	...
	while(iter) {
		...
		cfg.flows.forEach(f=>{
			if(
				(f.id==idFlow && !flowSet.has(f.id)) ||
				(flowSet.has(f.fparent) && !flowSet.has(f.id))
			){
				flowSet.add(f.id);
				iter = true;
			}
	...
	cfg.inputs = Object.keys(cfg.inputs).reduce((map,key)=>{
		...
		if(input.attach && input.attach.some(idFlow=>flowSet.has(idFlow)))
			map[key] = input;
		return map;
	},{});
	...
	cfg.modules.inputs = Object.keys(cfg.modules.inputs).reduce((map,key)=>{
		...
		if(input.$def.attach && input.$def.attach.some(idFlow=>flowSet.has(idFlow)))
			map[key] = input;
		return map;
	},{});

	cfg.flows = cfg.flows.filter(f=>flowSet.has(f.id));

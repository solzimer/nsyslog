...
async function read(file,callback,validateOnly) {
	...
	try {
		...
		dupes = [].concat(...dupes.filter(a=>a.length));
		dupes.forEach(dup=>{
			logger.warn(`Duplicate ID for`,dup);
		});

		//if(validateOnly) return json;
		await processInputs(json); 	// Get Inputs
		processFilters(json);				// Get Filters
		let ferrors = await processFlows(json);		// Get Flows

		ferrors.forEach(err=>logger.error(err));
		if(callback) callback(ferrors.length? errors : null,json,raw);
		else {
			if(ferrors.length) throw ferrors;
			else return json;
		}
	}catch(err) {
 ...
 */
async function imports(file,json,dupes) {
	dupes = dupes || [];
	...
	let all = (json.include || []).map(async f=>{
		...
		await registerComponents(basepath,njson);
 		njson = await imports(ifile,njson,dupes);

		// Check duplicated IDs
		dupes.push(checkDuplicates(json,njson));
...
function checkDuplicates(json1,json2) {
	...
	function match(type,vals1,vals2) {
		...
		Object.keys(vals2||{}).forEach(k=>{
			if(map.has(k)) duplicates.push({type,id:k,path:vals2[k].$$filename});
			else map.add(k);
		});
	}
...
async function processFlows(json) {
	...
	const
		...
		nof = function(f){errors.push(`Filter '${f}' doesn't exist`); return voidfn;},
		nofg = function(f){errors.push(`Filter Group '${f}' doesn't exist`); return voidfn;},
		nop = function(f){errors.push(`Processor '${f}' doesn't exist`); return voidpr(f);},
		nopg = function(f){errors.push(`Processor Group '${f}' doesn't exist`); return voidpr(f);},
		notr = function(tr){errors.push(`Transporter '${tr}' doesn't exist`); return voidtr(tr);},
		notrg = function(tr){errors.push(`Transporter Group '${tr}' doesn't exist`); return voidtr(tr);},
		vf = function(val){
			try{
				...
				errors.push("Error parsing expression:",val,err);
				return ()=>false;
			}
		};

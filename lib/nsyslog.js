...
class NSyslog extends EventEmiter {
	constructor(cfg) {
		...
		Reemiter.setStream(this.inputStream,entry=>{
			this.emit('all','data','input',null,{instance:{id:"#"}},entry);
			this.emit('data','input',null,{instance:{id:"#"}},entry);
		});
	}
	...
	startProcessorStream() {
		...
		cfg.flows.forEach(flow=>{
			...
			// Forked flow
				...
				let p = FlowFork.fork(this.config.$file,flow.id);
				this.attachEvents(p,flow,'processor');
				to = to.pipe(p);
			}
	...
	startFlowStream() {
		...
		flows.forEach((f,i)=>{
			f.id = f.id || `Flow_${i}`;
			let wstr = new FilterStream(`${f.id}_Entry_Point`,(x)=>f.from(x) && f.when(x));
			f.stream = wstr;
	...
	startInputs() {
		...
		Object.keys(cfg.modules.inputs).forEach(k=>{
			...
			if(!this.forked) {
				if(m.$def.attach && m.$def.attach.length) return;
			}

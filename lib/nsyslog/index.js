...
class NSyslog extends ClusterAware {
	...
	async startChildren() {
		...
		this.children = Object.
			...
			map(pk=>FlowFork.fork(this.allconfigs[pk],pk));
	}
	...
	async startFlowStream() {
		...
		// Active flows
		let flows = cfg.modules.flows.filter(f=>!f.disabled);
		...
		logger.info(`Process ${process.pid} registers flows:`,flows.map(f=>f.id));
		flows.forEach(f=>Shm.hpush('flows',f.id,this.pubsub.link));
	...
	async start() {
		...
		logger.info(`Process [${process.pid}] will start with the follogin components:`);

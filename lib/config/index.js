...
async function imports(file,json,errors) {
	...
	let all = (json.include || []).map(async f=>{
		let ns = f.ns || false;
		let ifile = path.resolve(basepath,f.path||f);
		...
		// Set filename on each module and assign namespaces
		(njson.flows||[]).forEach(f=>f.$$filename=ifile);
		M_TYPES.forEach(pk=>{
			Object.keys(njson[pk]||{}).forEach(k=>{
				let id = ns? `${ns}:${k}` : k;
				njson[pk][id] = njson[pk][k];
				njson[pk][id].$$filename=ifile;
				if(id!=k) delete njson[pk][k];

				if(pk=='processorGroups' && id!=k) {
					njson[pk][id] = njson[pk][id].map(pr=>{
						if(!pr.startsWith('$')) return `${ns}:${pr}`;
					});
				}
				else if(pk=='transporterGroups' && id!=k) {
...
async function processFlows(json) {
	...
	const
		...
		voidtr = (id)=>Transporters.instance(id,"null"),
		voidpr = (id)=>Processors.instance(id,"null"),

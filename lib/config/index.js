	...
	split() {
		...
		Object.keys(this.inputs).forEach(ki=>{
			...
			Object.keys(modules.inputs).forEach(k=>{
				if(!pmap[MASTER_PROC].cfg.inputs[k]) delete modules.inputs[k];
			});
		});
		...
		Object.keys(pmap).forEach(pk=>{
			...
			Object.keys(modules.processors).forEach(k=>{
				if(!pmap[MASTER_PROC].cfg.processors[k]) delete modules.processors[k];
			});
		});
		...
		Object.keys(pmap).forEach(pk=>{
			...
			Object.keys(modules.transporters).forEach(k=>{
				if(!pmap[MASTER_PROC].cfg.transporters[k]) delete modules.transporters[k];
			});
		});
		...
		// Filter master flows
		modules.flows = pmap[MASTER_PROC].flows;
		...
		pmap[MASTER_PROC].modules = modules;

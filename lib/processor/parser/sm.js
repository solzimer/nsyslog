		...
		while(true) {
			...
			if(!match && this.state!=START) {
				...
				if(!rule.name || !rule.name.length) {
					...
					if(!obj[rule.name]) obj[rule.name] = val.trim();
				}
					...
					if(rule.name.length==1) {
						...
						for(let i=1;i<match.length;i++) {
							...
							if(!rule.name[i-1]) continue
							results.push({name:rule.name[i-1], value:val.trim()});
							if(!obj[rule.name[i-1]]) obj[rule.name[i-1]] = val.trim();
						}
					}
				}
				...
				if(rule.set && rule.set.length) {
					rule.set.forEach(s=>{
						results.push({name:s.key, value:s.value(obj)});
					});
				}
			}
		}

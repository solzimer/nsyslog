...
class WindowsInput extends Input {
			...
			child.stdout.on('data', async (data) => {
				...
				jslines.filter(j=>j!=null).forEach(json=>{
					...
					if(json.Event.EventData && json.Event.EventData.Data) {
						...
						else if(Array.isArray(edata) && edata.length && !edata[0].Name) {
							json.Event.EventData.Data = edata;
						}
						else if(!Array.isArray(edata.reduce)) {
							json.Event.EventData.Data = edata;
						}
						else {
	...
	async setupWatermark() {
		...
		if(!this.wm[this.channel]) {
			logger.debug(`Initialize watermark for Windows channel ${this.channel}`);
			this.wm[this.channel] = await this.initWatermark();
			wm = this.wm[this.channel];
			exists = false;
		}
		else {
			logger.debug(`Watermark for Windows channel ${this.channel} exists in db`);
			wm = this.wm[this.channel];
			exists = await fexists(wm.bm);
			logger.debug(`Watermakr is valid: ${exists}`);
		}

...
async function start(path, idFlow) {
	...
	instance.on('data',(stage,flow,module,entry)=>{
		process.send({module:MODULE,cmd:CMD.event, event:'data', args : [stage,flow.id,module.instance.id]});
	});

	instance.on('error',(stage,flow,module,error)=>{
		process.send({module:MODULE,cmd:CMD.event, event:'error', args : [stage,flow.id,module.id,error.message]});
	});

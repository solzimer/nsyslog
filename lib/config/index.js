...
function extendConfig(json) {
	...
	Object.keys(modules.inputs).forEach(id=>{
		...
		input.buffer = json.inputs[id].buffer || false;
		input.when = extend({},{filter:false,bypass:false},json.inputs[id].when);
	});
	...
	Object.keys(modules.processors).forEach(id=>{
		...
		pr.when = extend({},{filter:false,bypass:false},json.processors[id].when);
	});
	...
	Object.keys(modules.transporters).forEach(id=>{
		...
		tr.when = extend({},{filter:false,bypass:false},json.transporters[id].when);
	});
}
